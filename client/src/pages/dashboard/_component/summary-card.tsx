...
const getCardStatus = (value: number, cardType: CardType, expenseRatio?: number): CardStatus => {
  if (cardType === "savings") {
    ...
    if (value < 10) return { label: "Low Savings", color: "text-red-400", Icon: TrendingDownIcon, description: `Only ${value.toFixed(1)}% saved` };
    if (value < 20) return { label: "Moderate", color: "text-yellow-400", Icon: TrendingDownIcon, description: `${expenseRatio?.toFixed(0)}% spent` };
    if (expenseRatio && expenseRatio > 75) return { label: "High Spend", color: "text-red-400", Icon: TrendingDownIcon, description: `${expenseRatio.toFixed(0)}% spent` };
    if (expenseRatio && expenseRatio > 60) return { label: "Warning: High Spend", color: "text-orange-400", Icon: TrendingDownIcon, description: `${expenseRatio.toFixed(0)}% spent` };
  ...
  if (cardType === "balance" && value < 0) return { label: "Overdrawn", color: "text-red-400", Icon: TrendingDownIcon, description: "Balance is negative" };

  return { label: "", color: "", Icon: TrendingDownIcon };
};

const getTrendDirection = (value: number, cardType: CardType) => cardType === "expenses" ? (value <= 0 ? "positive" : "negative") : value >= 0 ? "positive" : "negative";
      ...
      <CardContent className="space-y-5">
        ...
        <div className="text-sm text-muted-foreground mt-2">
          {cardType === "savings" ? (
            ...
            <div className="flex items-center gap-1.5">
              {percentageChange !== 0 && (
                <div className={cn("flex items-center gap-0.5", trendDirection === "positive" ? "text-green-500" : "text-red-500")}>
                  {trendDirection === "positive" ? <TrendingUpIcon className="size-3" /> : <TrendingDownIcon className="size-3" />}
                  <span>{formatPercentage(percentageChange || 0, { showSign: percentageChange !== 0, isExpense: cardType === "expenses", decimalPlaces: 1 })}</span>
                </div>
              )}
              {percentageChange === 0 && (
